...
class BuyBillController extends Controller
{
    public function add_serials(Request $request)
    {
        ...
        echo '<form id="myForm" method="POST" action="/client/save-serials-buy" target="_blank">';
        echo '<input type="hidden" name="_token" value="' . csrf_token() . '">';
        echo '<input type="hidden" name = "product_unit_id" value="' . $product_unit_id . '" /> ';
        echo '<input type="hidden" name = "buy_element_id" value="' . $element_id . '" /> ';
        echo '<input type="hidden" name = "buy_bill_id" value="' . $buy_bill_id . '" /> ';
    ...
    public function save_serials(Request $request)
    {
        ...
        $buy_element_id = $request->buy_element_id;
        $buy_element = BuyBillElement::FindOrFail($buy_element_id);
        $old_serials = $buy_element->serials;
        ...
        for ($i = 0; $i < $quantity; $i++) {
            $buy_serial = BuySerial::create([
                'company_id' => $company_id,
                'client_id' => $client_id,
                'product_unit_id' => $product_unit_id,
                'buy_bill_id' => $buy_bill_id,
                'buy_element_id' => $buy_element_id,
        ...
        <script>
            $('.remove_element').on('click',function(){
                ...
                $.post('/client/buy-bills/discount',
                    {'_token': '" . csrf_token() . "',buy_bill_number:buy_bill_number, discount_type: discount_type, discount_value: discount_value},
                    function (data) {
                        $('.after_totals').html(data);
                });

                $.post('/client/buy-bills/extra',
                    {'_token': '" . csrf_token() . "',buy_bill_number:buy_bill_number, extra_type: extra_type, extra_value: extra_value},
                    function (data) {
                        $('.after_totals').html(data);
                });
