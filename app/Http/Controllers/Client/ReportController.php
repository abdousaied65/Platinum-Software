...
class ReportController extends Controller
{
    ...
    public function post_report1(Request $request)
    {
        ...
        $outer_clients = $company->outerClients;
        return view('client.reports.report1', compact('outer_clients', 'posBills', 'company', 'company_id', 'saleBills', 'currency', 'outer_client_id'));
    }
    ...
    public function post_report2(Request $request)
    {
        ...
        return view('client.reports.report2',
            compact('products', 'company', 'posBills', 'company_id', 'saleBills', 'currency', 'product_id'));
    }
    ...
    public function post_report3(Request $request)
    {
        ...
        $suppliers = $company->suppliers;
        return view('client.reports.report3', compact('suppliers', 'company', 'company_id', 'buyBills', 'currency', 'supplier_id'));
    }
    ...
    public function post_report4(Request $request)
    {
        ...
        return view('client.reports.report4',
            compact('products', 'company', 'company_id', 'buyBills', 'currency', 'product_id'));
    }
    ...
    public function get_clients_by_branch_id(Request $request)
    {
    ...
    public function get_report13()
    {
        ...
        return view('client.reports.report13', compact('company', 'branches'));
    }
    ...
    public function post_report13(Request $request)
    {
        ...
        $branches = $company->branches;
        $branch_id = $request->branch_id;
        $client_id = $request->client_id; // array()

        if (empty($from_date) || empty($to_date)) {
            $sale_bills = SaleBill::where('company_id', $company_id)->WhereIn('client_id',$client_id)->get();
            $pos_bills = PosOpen::where('company_id', $company_id)->WhereIn('client_id',$client_id)->get();
            $cashs = Cash::where('company_id', $company_id)->WhereIn('client_id',$client_id)->get();
            $bank_cashs = BankCash::where('company_id', $company_id)->WhereIn('client_id',$client_id)->get();
        } else {
            ...
            $cashs = Cash::where('company_id', $company_id)
                ...
                ->get();
            $bank_cashs = BankCash::where('company_id', $company_id)->WhereIn('client_id',$client_id)
                ->whereBetween('created_at', [$from_date, date('Y-m-d', strtotime($to_date . ' +1 day'))])
                ->get();
        }
        return view('client.reports.report13',
            compact('company', 'sale_bills', 'pos_bills','cashs','bank_cashs', 'company_id', 'branches', 'branch_id','client_id', 'from_date', 'to_date', 'currency'));
    }
    ...
    public function post_report16(Request $request)
    {
        ...
        return view('client.reports.report16',
            compact('company', 'company_id', 'safes_balances', 'banks_balances', 'total_purchase_prices', 'result', 'outerClients', 'suppliers', 'sale_bills', 'cashs', 'buy_cashs',
                'capitals', 'expenses', 'pos_bills', 'buy_bills', 'currency'));
    }
        ...
        if (isset($submit) && !empty($submit) && $submit == "best_sales") {
            ...
            foreach ($products as $product) {
                ...
                $sel = SaleBillElement::where('product_id', $product_id)
                    ->sum('quantity');
                $sum = $sum + $sel;
                $sel = PosOpenElement::where('product_id', $product_id)
                    ->sum('quantity');
                $sum = $sum + $sel;
            ...
            return view('client.reports.report18',
                compact('company', 'count', 'submit', 'count_arr', 'products_arr', 'final_sales', 'company_id', 'currency'));
            ...
            foreach ($products as $product) {
                ...
                $quantity_price = PosOpenElement::where('product_id', $product_id)
                    ->sum('quantity_price');
                $quantity_prices = $quantity_prices + $quantity_price;
            ...
            return view('client.reports.report18',
                compact('company', 'count', 'submit', 'profit_arr', 'final_profits', 'products_arr', 'company_id', 'currency'));
        }
    }
    ...
    public function post_report19(Request $request)
    {
        ...
        $pos_elements = PosOpenElement::where('product_id', $product_k->id)->get();
        ...
        return view('client.reports.report19', compact('stores', 'product_id', 'products', 'total_sold',
    ...
    public function post_report20(Request $request)
    {
        ...
        return view('client.reports.report20',
            compact('safes', 'safes_transfers', 'safe_id', 'safe_k', 'company_id', 'company', 'bank_safe_transfers', 'safe_bank_transfers'
    ...
    public function post_report21(Request $request)
    {
        ...
        if (!$stores->isEmpty()) {
            $products_k = Product::whereIn('store_id', $stores_ids)->get();
        }

        return view('client.reports.report21',
            compact('branch_id', 'branch_k', 'company_id', 'company',
    ...
    public function get_report22()
    {
        ...
        return view('client.reports.report22', compact('clients', 'company_id', 'company'));
    }
    ...
    public function post_report22(Request $request)
    {
        ...
        $reports = ShiftReport::where('client_id', $client_id)
            ->whereBetween('created_at', [$from_date, date('Y-m-d', strtotime($to_date . ' +1 day'))])
            ->get();
        return view('client.reports.report22',
            compact('client_id', 'clients', 'client', 'company_id', 'company',
                'from_date', 'to_date', 'reports'));
    }
}
